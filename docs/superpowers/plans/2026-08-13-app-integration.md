...
Keep one stable injected `ConversationDependencies` value. Initialize `_viewModel = State(initialValue: ConversationViewModel(dependencies: dependencies))` inside `AppRootView.init(dependencies:)`; live dependencies are constructed once at app-root ownership, not during `body` updates. Show `OnboardingView`, and enter `ConversationView` only after its explicit start action. `ConversationViewModel` starts and owns the one retained audio-event consumer in its initializer (or an idempotent internal start), using weak capture so the task cannot retain the view model forever. `AppRootView` never subscribes to audio events. `shutdown()` cancels and awaits that consumer before calling `serviceTeardown`.
