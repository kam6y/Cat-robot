...
Forward `.inactive`/`.background` scene phases to `sceneBecameInactive` idempotently; do not auto-resume on `.active` and do not invoke service teardown from scene-phase changes. Bind UI actions to view-model methods using token-aware `Task` calls, including `updateTypedText(_:)` and the UI contract's explicit `hideTypedInput()` callback so the parent remains the source of truth. The send action snapshots `viewState.typedText` synchronously before creating its async `Task`, so the UI's immediate dismiss cannot erase the submitted value. Route recovery actions explicitly: retry calls `retryRecovery()`, typed input calls `showTypedInput()`, and Settings uses SwiftUI's `openURL` with `UIApplication.openSettingsURLString`. Disable the application idle timer only for the visible conversation lifetime and restore its prior value when that view disappears so the StandBy-like face remains awake without leaking the setting. Do not map scene inactivity, interruption, or route change to `shutdown()`; those retain prepared Speech assets for low-latency explicit resume. Terminal cleanup remains the idempotent view-model API used by tests and any future explicit exit route; process termination itself does not depend on asynchronous cleanup completing because iOS reclaims process-owned resources.

- [ ] **Step 3: Add signposts and README limitations**

Record the last ASR activity timestamp and the 1.2-second segmentation interval separately from model/TTS work. Start response signposts when the app commits the detected utterance boundary; end them on first reply snapshot and `.started` speech event, and report both post-boundary and approximate last-ASR-activity latency so the silence wait is not hidden. Store no content in logs. Keep final-only TTS for MVP correctness, but keep live replies to a short spoken answer and evaluate sentence-prefix TTS only if real-device audible latency misses the target. README explicitly states: foreground only, no speaker ID, no barge-in, no background listening, addressee detection can be wrong, AI replies can be wrong, and all MVP processing is on device.
