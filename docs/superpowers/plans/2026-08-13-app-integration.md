...
**Architecture:** One `@MainActor @Observable` `ConversationViewModel` owns UI state plus distinct capture, segmentation, turn, preflight, and audio-event tasks. Monotonic session/capture/turn identities prevent stale callbacks from mutating a newer conversation. It stops capture at each utterance boundary, takes deterministic wake/engagement fast paths before optional classification, streams one stateful reply into captions, speaks it, and resumes capture only after normal TTS completion; pause/background/interruption/audio-route change require explicit resume.
...
- Pause/background/interruption/audio-route change clears engagement and pending clarification and never silently resumes.
- Pause/background/interruption/audio-route change performs per-turn stop/deactivation only; it does not release prepared Speech assets. Run the explicit service teardown only when the conversation screen or its app-root owner is actually leaving.
...
- Use real device `Not so bad`, UDID `00008140-000610311A90801C`, iOS 26.6; the validated iPhone 16 Pro simulator UDID is `1C7D7250-8926-453F-A714-820FB9C3D9F7`.
...
The view model serializes all public actions through one lifecycle generation. It is the sole owner of separate cancellable `preflightTask`, `captureTask`, `segmentationTask`, `turnTask`, and `audioEventTask`, plus monotonic session/capture/turn counters. Repeated start/resume calls join or ignore the current preflight; pause/background/interruption/audio-route change invalidates the generation before awaiting child teardown. Every recognition event, timer firing, model snapshot, speech event, audio event, and preflight completion verifies its captured identity before changing state or restarting capture. At an utterance boundary, mark that capture as closing, cancel its segmentation timer, and prevent any second turn from starting, but keep its stream consumer and capture identity valid until the recognizer's lossless `stop()` and the consumer both finish. Fold any final tail into that same utterance exactly once, then invalidate the capture identity before routing it. The view model lazily starts its sole audio-event consumer on the first explicit conversation or typed-turn start, not during onboarding construction; the idempotent `shutdown()` cancels and joins it before concrete service teardown. `AppRootView` never opens a second subscription.
...
Run: `ruby scripts/generate_project.rb && xcodebuild test -project CatRobot.xcodeproj -scheme CatRobot -destination 'platform=iOS Simulator,id=1C7D7250-8926-453F-A714-820FB9C3D9F7' -only-testing:CatRobotTests/ConversationErrorPresentationTests -derivedDataPath .build/DerivedData CODE_SIGNING_ALLOWED=NO`
...
**Files:**
- Modify: `CatRobot/Conversation/Services/AppleSpeechRecognizer.swift`
...
Before the view-model tests, add a focused recognizer lifecycle regression: `stop()` from a successfully `.prepared` state immediately cancels that route-bound driver, returns the recognizer to `.unprepared`, retains its Speech asset reservation, and makes the next `prepare()` create and prepare a fresh driver. This closes the route-change-during-preflight gap without performing terminal `shutdown()`.
...
`ConversationHarness.emitCompletedUtterance` feeds a final recognition event and advances a test-only `flushSegmentation(at:)` seam by 1.2 seconds; production schedules the next flush at the earlier of the 1.2-second silence deadline and the 20-second hard deadline, cancelling/rescheduling only the silence side on new activity. Also evaluate the hard deadline after every recognition event so continuous provisional activity cannot defer it indefinitely.

- [ ] **Step 2: Run tests; expect missing view model/harness**

- [ ] **Step 3: Implement start and happy-path turn-taking**

`startConversation` requests permission, checks model availability, prepares the installed Japanese synthesis voice, activates audio, then prepares route-bound speech recognition, prewarms reply, and starts recognition. Audio activation must precede every `AppleSpeechRecognizer.prepare()` because stopping returns its internal driver to an unprepared route-bound state; initial start and explicit resume therefore keep the order `activate → prepare → start`. Repeated starts while preflight is pending join or no-op; a permission/preflight completion from an invalidated generation cannot publish success or failure.

A completed utterance marks its capture as closing and cancels its segmentation timer, but keeps the recognition consumer alive while awaiting the recognizer's lossless stop. Tail finals received during closing are appended to that same segmenter without scheduling another timer; after `stop()` and the consumer finish, flush once, then invalidate the capture identity before starting routing or any newer recognition stream. Before handling any accepted route, consume and clear the current pending clarification; this applies to `.wakeOnly`, `.accept`, and a classified `.addressed` result, and happens before local acknowledgement or reply generation. `.wakeOnly` immediately captions and locally speaks the fixed phrase `なあに？`, never calls the classifier or reply session, arms engagement after acknowledgement finishes, and resumes capture. `.accept` goes directly to `streamReply`; `.classify` calls the classifier once. A classified `.addressed` result enters reply generation and arms engagement after the reply; `.ambiguous` enters the clarification flow; `.notAddressed` sends nothing to the reply session and starts a fresh recognition stream only after prior capture teardown completes. Replace caption with each cumulative snapshot. Speak only the final nonempty snapshot. On speech word events cycle `small/medium/wide`; on finish arm engagement for an explicit wake, confirmed pending utterance, or classified address, otherwise refresh an already active engagement; reset mouth and start one fresh recognition stream.

Add focused race tests for two concurrent starts; scene inactivity while permission/preflight is suspended; pause during reply streaming; background during TTS; a tail final received while capture is closing (it is included once in the current utterance and never seeds a second turn); stale model snapshot or speech-finished event after a newer session; duplicate interruption/inactive events; and rapid pause/resume. Each proves stale callbacks do not update the UI, arm engagement, or restart capture.
...
For `.ambiguous`, retain only one `PendingClarification`, speak the fixed local question without sending it to the reply session, and wait for yes/no. Negative/timeout discards it. A non-yes/no response to an active clarification discards the old pending utterance and routes the current utterance as a fresh `.classify` turn; never repeat the fixed clarification automatically. Any accepted route consumes and clears the pending clarification before local acknowledgement or reply generation, including an affirmative acceptance of its original utterance and a new explicit wake-name turn that supersedes it; an old pending utterance must never reappear after a fresh accepted turn. Add a domain test for this natural-correction behavior. An empty finalized recognition event presents `.speechUnrecognized` with **もう一度** and **文字で入力** while capture remains in `.listening`; `retryRecovery()` clears this card without restarting the already-running capture, and `showTypedInput()` opens the fallback. For paused or failed availability states, `retryRecovery()` reruns the same preflight as explicit resume. `pause`, scene inactivity, any audio interruption, and `.routeChanged` cancel timer/model/stream tasks, stop recognizer/speaker, deactivate audio, and clear engagement/pending state, but deliberately retain the Speech asset reservation for low-latency explicit resume. Route change is treated as a lifecycle invalidation because `AppleSpeechRecognizer.prepare()` captured the old input format; only an explicit resume may reactivate audio and prepare the recognizer against the new route. `toggleListening()` from paused reruns model availability, speech-asset preparation, synthesis-voice preparation, and audio activation before starting capture; a failed preflight stays visibly failed/paused and never pretends to listen. Context exceeded resets the reply session once and shows that short-term conversation memory was reset; it does not retry the same prompt silently.

Typed text bypasses microphone permission, recognition preparation, segmentation, and addressee classification. It still performs a typed-only preflight of model availability, reply prewarming, speech voice preparation, and audio activation before generation/TTS. If capture is running, close it losslessly first; if microphone permission had failed, finish the typed response in `.paused` instead of pretending to listen. Snapshot the submitted text before the UI synchronously dismisses its editor, clear it only after capture, and ignore duplicate submissions while classifying/thinking/speaking. Preserve user-owned typed text, editor visibility, and the last useful caption across generic phase-state transitions unless that transition explicitly changes them. Add a microphone-denied typed test proving availability/reply/speaker/audio run, recognizer prepare/start stay at zero, and the generated caption remains visible. A successful typed turn may resume recognition only when the same foreground lifecycle previously had microphone permission and listening enabled.
...
- Create: `CatRobot/Conversation/Integration/ConversationScreenWakeLock.swift`
...
Verify `AppRootView(dependencies:)` can be initialized with fakes and starts in onboarding without requesting microphone access or starting any service. Also test that an audio-session interruption reaches the view model, ordinary `.active` does not resume, and duplicate `.inactive`/`.background` forwarding is idempotent. Cover the initial microphone system prompt explicitly: the transient `.inactive` while an explicit start/permission request is pending is deferred, `.active` allows that same start to finish exactly once after the user grants access, while `.background` during the prompt invalidates the start and leaves the app paused. The MVP has no in-app back/exit route: app-root ownership is the conversation lifetime, while backgrounding is only a pause. Keep idempotent terminal teardown covered directly by `ConversationViewModel.shutdown()` tests plus the Apple-services composition test proving that the same closure reaches `AppleSpeechRecognizer.shutdown()` and releases one successful Speech reservation exactly once. Inject a token/ref-count wake-lock adapter and prove it disables automatic screen locking only while the conversation screen is visible and active, restores the pre-existing idle-timer value on the final release, and tolerates duplicate acquire/release without changing a newer owner's state.

- [ ] **Step 2: Implement root composition**

Keep one stable injected `ConversationDependencies` value. Initialize `_viewModel = State(initialValue: ConversationViewModel(dependencies: dependencies))` inside `AppRootView.init(dependencies:)`; live dependencies are constructed once at app-root ownership, not during `body` updates. Show `OnboardingView`, and enter `ConversationView` only after its explicit start action. `ConversationViewModel` lazily starts and owns one retained audio-event consumer from its first explicit conversation/typed start, using weak capture so the task cannot retain the view model forever; onboarding construction performs no permission, model, audio, speech, or event-subscription work. `AppRootView` never subscribes to audio events. `shutdown()` cancels and awaits that consumer before calling `serviceTeardown`.

Forward `.background` to `sceneBecameInactive` immediately and idempotently. Defer the first transient `.inactive` only while an explicit start is awaiting microphone permission; `.active` releases that pending-start gate but never starts or resumes a normally paused conversation. Any real `.background` while the prompt is visible invalidates the pending start. Outside that narrow permission-prompt window, forward `.inactive` idempotently as a pause. Do not invoke service teardown from scene-phase changes. Bind UI actions to view-model methods using token-aware `Task` calls, including `updateTypedText(_:)` and the UI contract's explicit `hideTypedInput()` callback so the parent remains the source of truth. The send action snapshots `viewState.typedText` synchronously before creating its async `Task`, so the UI's immediate dismiss cannot erase the submitted value. Route recovery actions explicitly: retry calls `retryRecovery()`, typed input calls `showTypedInput()`, and Settings uses SwiftUI's `openURL` with `UIApplication.openSettingsURLString`. Acquire a token/ref-count idle-timer lease only while the conversation screen is visible and the scene is active; snapshot `UIApplication.shared.isIdleTimerDisabled` on the first acquire and restore that exact prior value on the final release so the StandBy-like face remains awake without leaking or clobbering another owner's setting. Do not map scene inactivity, interruption, or route change to `shutdown()`; those retain prepared Speech assets for low-latency explicit resume. Terminal cleanup remains the idempotent view-model API used by tests and any future explicit exit route; process termination itself does not depend on asynchronous cleanup completing because iOS reclaims process-owned resources.

- [ ] **Step 3: Add signposts and README limitations**

Record the last ASR activity timestamp and the 1.2-second segmentation interval separately from model/TTS work. Give every accepted utterance its own latency token: start response signposts when the app commits that detected utterance boundary, end first-caption once on the first nonempty reply snapshot (or the fixed local wake acknowledgement), and end speech-start once on the matching `.started` event. Use separate signpost IDs and the exact literal names `FastPathFirstCaption`, `ClassifiedFirstCaption`, `FastPathSpeechStart`, and `ClassifiedSpeechStart`. Ignored, ambiguous, failed, paused, backgrounded, interrupted, route-changed, typed-replaced, or shut-down turns close/cancel their own open intervals, and stale callbacks cannot close a newer turn's intervals. Report both post-boundary and approximate last-ASR-activity latency so the silence wait is not hidden, and store no transcript or reply content in logs. Keep final-only TTS for MVP correctness, but keep live replies to a short spoken answer and evaluate sentence-prefix TTS only if real-device audible latency misses the target. README explicitly states: foreground only, no speaker ID, no barge-in, no background listening, addressee detection can be wrong, AI replies can be wrong, and all MVP processing is on device.
