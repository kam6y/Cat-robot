...
iPhoneの二文中心の比較では、返答開始の中央値が1.55秒から1.13秒へ約27%短縮しました。事前の自動採用条件「0.5秒以上短縮」は未達でしたが、2026-09-25に利用者が結果を踏まえて採用を承認したため有効化しました。長い返答での短縮量は未測定で、分類経路の標本不足・発熱・全件試聴と外部録音の未検証も残ります。[測定結果と採用判断](docs/validation/2026-09-23-conversation-response-latency.md)を参照してください。
