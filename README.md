...
最初の一文を生成途中から読み上げる候補方式を実装しています。現時点の通常アプリは、全文の生成・保存が完了してから読み上げる方式を維持しています。iPhoneで音声セッションを開始できず、速度・試聴の比較が成立していないため、候補方式の採用は保留です。[評価結果と採用条件](docs/validation/2026-09-23-conversation-response-latency.md)を参照してください。
