...
最初の一文を生成途中から読み上げる候補方式を実装しています。現時点の通常アプリは、全文の生成・保存が完了してから読み上げる方式を維持しています。iPhoneでの比較では返答開始の中央値が1.55秒から1.13秒へ約27%短縮しましたが、事前の採用条件「0.5秒以上短縮」には届かず、今回は有効化していません。分類経路の成功標本不足と発熱、全件試聴・外部録音の未検証も残っています。[評価結果と採用条件](docs/validation/2026-09-23-conversation-response-latency.md)を参照してください。

候補方式では、最初の一文と残りを順に読み上げ、生成・保存・読み上げが全て終わってから聞き取りを再開します。全文の確定前に中断した返答は保存せず、確定後は音声が失敗・停止しても全文を保存します。要約と履歴の再構築による待ち時間は残ります。
