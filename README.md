...
- 音声入力には日本語の`SpeechAnalyzer` / `SpeechTranscriber`、読み上げには端末内のSupertonic 3を使います。初期設定はF1で、会話画面の「…」→「声を選ぶ」から全10種類を選択できます。選択は再起動後も保持します。
...
通常アプリはSupertonic 3を使用します。会話画面右上の「…」メニューで、「会話を忘れる」と並ぶ「声を選ぶ」からF1〜F5・M1〜M5を選べます。設定を開くと会話を一時停止し、閉じても自動再開しません。次に会話を再開した際に選んだ声を使います。会話の記憶は変更しません。
